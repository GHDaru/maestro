...
const slugDe = (arq) => basename(arq).replace(/\.md$/i, "").toLowerCase();
...
// sumario.html — índice navegável do livro (cartões por parte).
...
const corpoSumario =
  `<h1>Sumário</h1><p class="s-sub">${sumario.subtitulo}</p>` +
  sumario.partes.map((p) => `<div class="s-parte">${p.nome}</div><div class="s-grid">${p.itens.map(cartao).join("")}</div>`).join("");
